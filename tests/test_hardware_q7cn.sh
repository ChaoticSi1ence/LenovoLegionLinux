#!/bin/bash
# =============================================================================
# Comprehensive Hardware Test Script for Legion Pro 7 16IAX10H (Q7CN, EC 0x5508)
# =============================================================================
#
...
#
# Usage: sudo bash tests/test_hardware_q7cn.sh [OPTIONS]
#
# Options:
...
# === Pre-flight ===
check_root
echo -e "${BOLD}Legion Pro 7 16IAX10H (Q7CN) — Comprehensive Hardware Test${NC}"
echo -e "Date: $(date)"
...
if echo "$BIOS_VER" | grep -qi "Q7CN"; then
    pass "BIOS version matches Q7CN"
else
    warn "BIOS version '$BIOS_VER' does not contain Q7CN — this script is designed for Q7CN"
fi
...
# Generated by test_hardware_q7cn.sh on $(date)
#
...
if [ -n "$HWMON_DIR" ]; then
    ...
    if [ "$FAN_COUNT" -eq 3 ]; then
        ...
        warn "Only 2 fans detected. If this is Q7CN hardware, fan3 should be visible."
